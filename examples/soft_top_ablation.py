...
# 0 -> 1 -> 2 -> 3 -> 4; guaranteed non-cyclic seed
A_seed = np.zeros((N, N), bool)
for i in range(N-1):
    A_seed[i+1, i] = True  # edge i -> i+1
...
p2m = make_fixed_topology_p2m(spec, A_seed)
W0, th0 = p2m(params0)
...
# check whether the dynamics were actually frozen
W_used, _ = p2m(result.params)
# reconstruct adjacency from the ACTUAL weights used
A_from_W = np.asarray(W_used) > -1.2  # edges near -1+eps, non-edges near -1-delta
np.fill_diagonal(A_from_W, False)
print("A from actual W:\n", A_from_W.astype(int))
print("matches seed:", np.array_equal(A_from_W, A_seed))

G = np.asarray(block_gate_matrix(spec, result.params))
print("gates on masked edges:", G[A_seed])          # should all be ~1.0
print("gates off-mask:", G[~A_seed & ~np.eye(N,dtype=bool)].max())
print("W on masked edges:", np.asarray(W_used)[A_seed])
...
plt.tight_layout()
plt.savefig(output_dir / f"topo_abl_scale_N{N}.png", dpi=130)
print(f"saved topo_abl_scale_N{N}.png")
...
offmask = A_from_W & ~A_seed
print("new edges formed:", int(offmask.sum()))   # must be 0
print("acyclic:", not contains_directed_cycle(A_from_W, N))

rows, cols, *_ = spec.compile_indices()
print("rows:", np.asarray(rows))
print("cols:", np.asarray(cols))
print("A_fixed[rows, cols]:", A_seed[np.asarray(rows), np.asarray(cols)])
print("A_fixed[cols, rows]:", A_seed[np.asarray(cols), np.asarray(rows)])   # the transpose
...
plot_network_graph(
    ...
    save_path=str(output_dir / f"topo_abl_learned_graph_N{N}.png"),
)
...
if N == 3:
    ...
    before_n = plot_phase_portrait(
        ...
        save_path=f"images/phase_portraits/{N}-cycle_topo_abl_phase_before.png",
    )
    ...
    after_n = plot_phase_portrait(
        ...
        x0s=[x0],
        title=f"N={N} limit cycle (PCA projection)",
        save_path=f"images/phase_portraits/{N}-cycle_topo_abl_phase_after.png",
    )
    ...
    before_n_grid = plot_phase_grid(
        ...
        save_path=f"images/phase_portraits/{N}-cycle_topo_abl_grid_before.png",
    )
    ...
    after_n_grid = plot_phase_grid(
        ...
        x0s=[x0],
        title=f"N={N} limit cycle (slices)",
        save_path=f"images/phase_portraits/{N}-cycle_topo_abl_grid_after.png",
    )
...
def hardened_ctln_check(spec, params, p2m, x0, targets, *, dt, n_steps, burn_in, ridge):
    ...
    assert jnp.allclose(eps_j, eps_j[0]) and jnp.allclose(delta_j, delta_j[0])
...
def run_sweep(n_seeds=20, z_std=2.0):
    ...
    with open(out / "topo_abl_hyperparams.json", "w") as f:
    ...
    for seed in range(n_seeds):
        for z_std in [1.0, 2.0, 3.0]:
            ...
            params0_s = init_block_params_from_adjacency(spec, sub_s, A_seed)
            ...
            # save model params
            np.save(Path(__file__).parent / "results" / f"topo_abl_params_seed{seed}_z{z_std}.npy", np.asarray(res.params))
            ...
            with open(out / "topo_abl.jsonl", "a") as f:
    ...
    np.savez(out / "topo_abl_u.npz", u=np.concatenate(all_u))

    u_pooled = np.concatenate(all_u)
    np.save(Path(__file__).parent / "results" / "u_topo_abl_pooled.npy", u_pooled)
    plot_commitment_histogram(u_pooled, save_path=str(Path(__file__).parent / "images" / "commitment_hist.png"))

    out = Path(__file__).parent / "results"
    out.mkdir(exist_ok=True)
    with open(out / "topo_abl.json", "w") as f:
...
def plot_commitment_histogram(u_soft, u_direct=None, save_path="topo_abl_commitment_hist.png"):
    """
