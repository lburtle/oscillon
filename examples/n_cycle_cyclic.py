...
plt.tight_layout()
plt.savefig(output_dir / f"scale_N{N}_cyclic.png", dpi=130)
print(f"saved scale_N{N}_cyclic.png")
...
plot_network_graph(
    ...
    save_path=str(output_dir / f"learned_graph_N{N}_cyclic.png"),
)
...
if N == 3:
    ...
    before_3 = plot_phase_portrait(
        ...
        save_path="images/phase_portraits/3-cycle_phase_before_cyclic.png",
    )
    ...
    after_3 = plot_phase_portrait(
        ...
        save_path="images/phase_portraits/3-cycle_phase_after_cyclic.png",
    )
    ...
    before_3_grid = plot_phase_grid(
        ...
        x0s=x0s,
        title="3-cycle phase portrait (before)",
        save_path="images/phase_portraits/3-cycle_grid_before_cyclic.png",
    )
    ...
    after_3_grid = plot_phase_grid(
        ...
        x0s=x0s,
        title="3-cycle phase portrait (after)",
        save_path="images/phase_portraits/3-cycle_grid_after_cyclic.png",
    )
    ...
    before_n = plot_phase_portrait(
        ...
        save_path=f"images/phase_portraits/{N}-cycle_phase_before_cyclic.png",
    )
    ...
    after_n = plot_phase_portrait(
        ...
        x0s=[x0],
        title=f"N={N} limit cycle (PCA projection)",
        save_path=f"images/phase_portraits/{N}-cycle_phase_after_cyclic.png",
    )
    ...
    before_n_grid = plot_phase_grid(
        ...
        save_path=f"images/phase_portraits/{N}-cycle_grid_before_cyclic.png",
    )
    ...
    after_n_grid = plot_phase_grid(
        ...
        x0s=[x0],
        title=f"N={N} limit cycle (slices)",
        save_path=f"images/phase_portraits/{N}-cycle_grid_after_cyclic.png",
    )
...
def edge_commitment(params, spec, mode="soft"):
    ...
    stats = {
        "frac_ambiguous": float(np.mean((u >= 0.2) & (u <= 0.8))),
        "frac_committed": float(np.mean((u < 0.2) | (u > 0.8))),
...
def run_sweep(n_seeds=20, z_std=2.0):
    ...
    for seed in range(n_seeds):
        for z_std in [1.0, 2.0, 3.0]:
            ...
            with open(out / "cyclic.jsonl", "a") as f:
    ...
    np.savez(out / "cyclic_pop512_u.npz", u=np.concatenate(all_u))

    u_pooled = np.concatenate(all_u)
    np.save(Path(__file__).parent / "results" / "u_cyclic_pooled.npy", u_pooled)
    plot_commitment_histogram(u_pooled, save_path=str(Path(__file__).parent / "images" / "commitment_hist_cyclic.png"))

    out = Path(__file__).parent / "results"
    out.mkdir(exist_ok=True)
    with open(out / "cyclic_pop512.json", "w") as f:
...
def plot_commitment_histogram(u_soft, u_direct=None, save_path="commitment_hist_cyclic.png"):
    """
