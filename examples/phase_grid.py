...
def plot_phase_grid(
    ...
    trajs = [np.asarray(simulate(jnp.asarray(W), jnp.asarray(theta),
    ...
    for idx, (i, j) in enumerate(pairs):
        ...
        P[:, i] = GX.ravel(); P[:, j] = GY.ravel()
        F = _field_np(W, theta, P)
        U, V = F[:, i].reshape(GX.shape), F[:, j].reshape(GX.shape)
        mag = np.hypot(U, V)
        ax.streamplot(GX, GY, U, V, density=1.1, color=mag, cmap="viridis",
